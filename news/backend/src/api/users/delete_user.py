...
def delete_user(db: DBAdapter, jwt_secret_key: str, get_user_id: int):
    # Validate token
    ...
    # Check that we have written the user email address or something
    ...
    if sql_user_email != post_email:
        return {"message": f"Email is wrong", "data": None, "error": "Payment Required"}, 402

    # DELETE user
    query: str = """DELETE FROM n_users_index WHERE user_id=:user_id"""
    parameters: dict = {"user_id": sql_user_id}
    db.delete(query=query, parameters=parameters)

    # Give feedback
    return {"message": f"User deleted", "data": None, "error": ""}, 200
